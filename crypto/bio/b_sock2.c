...
# if defined(OSSL_TFO_CLIENT_SOCKOPT)
        if (setsockopt(sock, IPPROTO_TCP, OSSL_TFO_CLIENT_SOCKOPT,
            ...
            BIOerr(BIO_F_BIO_CONNECT, BIO_R_UNABLE_TO_TFO);
            return 0;
        }
# endif
...
#  if defined(OSSL_TFO_SYSCTL)
        ...
        if (sysctlbyname(OSSL_TFO_SYSCTL, &enabled, &enabledlen, NULL, 0) < 0) {
            BIOerr(BIO_F_BIO_LISTEN, BIO_R_TFO_NO_KERNEL_SUPPORT);
            return 0;
        } else if (!(enabled & OSSL_TFO_SERVER_FLAG)) {
            BIOerr(BIO_F_BIO_LISTEN, BIO_R_TFO_DISABLED);
            return 0;
        }
#  endif
        if (setsockopt(sock, IPPROTO_TCP, OSSL_TFO_SERVER_SOCKOPT,
                       (void*)&q, sizeof(q)) < 0) {
            SYSerr(SYS_F_SETSOCKOPT, get_last_socket_error());
            BIOerr(BIO_F_BIO_LISTEN, BIO_R_UNABLE_TO_TFO);
            return 0;
        }
    }
# endif
