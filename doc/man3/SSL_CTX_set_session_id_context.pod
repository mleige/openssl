...
SSL_CTX_get0_session_id_context() retreives the context into B<sid_ctx>
and the length into B<sid_ctx_len> within which a session can be reused
for the B<ctx> object.

SSL_get0_session_id_context() retreives the context into B<sid_ctx>
and the length into B<sid_ctx_len> within which a session can be reused
for the B<ssl> object.
