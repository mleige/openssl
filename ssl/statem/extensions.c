...
static EXT_RETURN tls_construct_compress_certificate(SSL *s, WPACKET *pkt, unsigned int context,
                                                     X509 *x, size_t chainidx)
{
#if !defined(OPENSSL_NO_COMP) && (defined(ZLIB) || defined(BROTLI))
...
# endif
            ...
            || !WPACKET_close(pkt)) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR,
                 SSL_F_TLS_CONSTRUCT_COMPRESS_CERTIFICATE, ERR_R_INTERNAL_ERROR);
        return EXT_RETURN_FAIL;
    }
                                   ...
                                   X509 *x, size_t chainidx)
{
    ...
    if (s->ext.compress_certificate_rx != TLSEXT_comp_cert_none) {
        SSLfatal(s, SSL_AD_DECODE_ERROR,
                 SSL_F_TLS_PARSE_COMPRESS_CERTIFICATE, SSL_R_DUPLICATE_EXTENSION);
        return 0;
    }
    ...
    if (!PACKET_as_length_prefixed_1(pkt, &supported_comp_algs)
        ...
        SSLfatal(s, SSL_AD_DECODE_ERROR,
                 SSL_F_TLS_PARSE_COMPRESS_CERTIFICATE, SSL_R_BAD_EXTENSION);
        return 0;
    }
